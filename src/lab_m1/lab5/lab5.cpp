...
// am o problema cu perspectiva ortografica
...
void Lab5::Update(float deltaTimeSeconds)
{
    ...
    {
        glm::mat4 modelMatrix = glm::mat4(1);
        modelMatrix = glm::translate(modelMatrix, glm::vec3(2, 4, -3));
        modelMatrix = glm::scale(modelMatrix, glm::vec3(3, 2, 1));
        modelMatrix = glm::rotate(modelMatrix, 1.54f, glm::vec3(1, 1, 0));
...
void Lab5::OnInputUpdate(float deltaTime, int mods)
{
    ...
    if (window->KeyHold(GLFW_KEY_M)) {
        // latimea se face mai mica
        latime += deltaTime * 2;
        inaltime += deltaTime * 2;
        projectionMatrix = glm::ortho(-latime, latime, -inaltime, inaltime, 0.01f, 200.0f);
    }
    if (window->KeyHold(GLFW_KEY_N)) {
        // latimea se face mai mare
        latime -= deltaTime * 2;
        inaltime -= deltaTime * 2;
        projectionMatrix = glm::ortho(-latime, latime, -inaltime, inaltime, 0.01f, 200.0f);
    }
