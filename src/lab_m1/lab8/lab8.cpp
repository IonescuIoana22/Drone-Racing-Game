    ...
    void Lab8::Update(float deltaTimeSeconds)
    {
        ...
        // Render ground
        {
            ...
            // TODO(student): Add or change the object colors
            RenderSimpleMesh(meshes["plane"], shaders["LabShader"], modelMatrix, glm::vec3(0.5f, 0.5f, 1));
    ...
    void Lab8::RenderSimpleMesh(Mesh *mesh, Shader *shader, const glm::mat4 & modelMatrix, const glm::vec3 &color)
    {
	    ...
	    int light_direction1 = glGetUniformLocation(shader->program, "light_direction1");
	    glUniform3f(light_direction1, lightDirection1.x, lightDirection1.y, lightDirection1.z);
        ...
        float cut_off_angle_location = glGetUniformLocation(shader->program, "cut_off_angle");
        glUniform1f(cut_off_angle_location, cut_off_angle);
    ...
    void Lab8::OnInputUpdate(float deltaTime, int mods)
    {
        ...
        if (!window->MouseHold(GLFW_MOUSE_BUTTON_RIGHT))
        {
            ...
            if (window->KeyHold(GLFW_KEY_UP)) lightPosition1 -= forward * deltaTime * speed;
            if (window->KeyHold(GLFW_KEY_LEFT)) lightPosition1 -= right * deltaTime * speed;
            if (window->KeyHold(GLFW_KEY_DOWN)) lightPosition1 += forward * deltaTime * speed;
            if (window->KeyHold(GLFW_KEY_RIGHT)) lightPosition1 += right * deltaTime * speed;
            if (window->KeyHold(GLFW_KEY_M)) lightPosition1 += up * deltaTime * speed;
            if (window->KeyHold(GLFW_KEY_N)) lightPosition1 -= up * deltaTime * speed;

            // TODO(student): Set any other keys that you might need
            
		    if (window->KeyHold(GLFW_KEY_U) && isSpotlight == 1) {
			    cut_off_angle -= deltaTime * 20;
		    }
            if (window->KeyHold(GLFW_KEY_O) && isSpotlight == 1) {
			    cut_off_angle += deltaTime * 20;
            }
