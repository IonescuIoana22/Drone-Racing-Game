...
namespace implemented
{
    class Camera_tema
    {
     public:
        ...
        void RotateCamera() {
            TranslateForward(glm::length(distance));
            if (move == 0) RotateFirstPerson_OX(-rotation.x);
            RotateFirstPerson_OY(rotation.y);
            if (move == 0) RotateFirstPerson_OZ(-rotation.z);
            TranslateForward(-glm::length(distance));
        }

        void SetRotation(glm::vec3 arg) {
            rotation = arg;
        }

        void SetDistance(glm::vec3 arg) {
            distance = arg;
        }
