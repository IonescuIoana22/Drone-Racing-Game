...
void Lab7::RenderSimpleMesh(Mesh *mesh, Shader *shader, const glm::mat4 & modelMatrix, const glm::vec3 &color)
{
	...
	GLint material_kd_pos = glGetUniformLocation(shader->program, "material_kd");
	glUniform1f(material_kd_pos, materialKd);

	GLint material_ks_pos = glGetUniformLocation(shader->program, "material_ks");
	glUniform1f(material_ks_pos, materialKs);

	GLint material_shininess_pos = glGetUniformLocation(shader->program, "material_shininess");
	glUniform1i(material_shininess_pos, materialShininess);

	GLint object_color_pos = glGetUniformLocation(shader->program, "object_color");
	glUniform3fv(object_color_pos, 1, glm::value_ptr(color));
